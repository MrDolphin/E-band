...
- 波长：`λ = c / fc ≈ 3.95 mm`；
- 距离分辨率：`ΔR = c / (2B) = 7.5 m`；
- 最大无模糊径向速度：`vmax = λ / (4Tchirp) ≈ 5.48 m/s`；
- 速度分辨率：`Δv = λ / (2NTchirp) ≈ 0.17 m/s`。

这些指标必须由配置对象实时计算，GUI不得维护第二套硬编码结果。由于复数IQ包含I和Q两路，30 MSPS表示每秒3000万个复数样点；按I/Q各16位传输时，原始数据量约120 MB/s。首版只采集有限CPI，不承诺持续传回30 MSPS原始IQ。
